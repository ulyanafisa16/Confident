"""
aidetection.py — AI Detection Engine (revised)
Revisi dari versi sebelumnya:
  - BlockedMimeTypeRule diperlonggar: hanya blokir file eksekusi murni,
    file script seperti .py .js .sql diizinkan karena use case sah
  - Tidak ada perubahan pada rule lain
  - Limit file diubah menjadi 50 MB untuk semua user

Ganti seluruh isi api/aidetection.py dengan file ini.
"""
...
# ===========================================================================
# RULES — BlockedMimeTypeRule direvisi
# ===========================================================================

class BlockedMimeTypeRule(Rule):
    """
    Hanya memblokir file yang benar-benar berupa program eksekusi.
    File script seperti .py, .js, .sql DIIZINKAN karena use case
    yang sah di platform confidential sharing (developer berbagi script).

    Yang diblokir:
    - File eksekusi Windows (.exe, .msi, .bat, .cmd, .scr, .pif, .com)
    - Shell script yang bisa langsung dieksekusi (.sh, .ps1)
    - File library sistem (.dll, .so, .dylib)
    - File Java executable (.jar, .war)

    Yang TIDAK diblokir (berbeda dari versi sebelumnya):
    - .py, .js, .ts, .rb, .pl  → script yang perlu shared secara aman
    - .php, .asp               → bisa jadi legitimate backup kode
    - .sql                     → sangat umum untuk backup database
    """
    ...
    # Hanya ekstensi yang benar-benar executable / berbahaya
    BLOCKED_EXTS = {
        # Windows executable
        ".exe", ".msi", ".com", ".pif", ".scr",
        # Script yang auto-run
        ".bat", ".cmd", ".ps1", ".ps2", ".vbs", ".vbe", ".wsf", ".wsh",
        # Library sistem
        ".dll", ".so", ".dylib",
        # Java executable
        ".jar", ".war", ".ear",
    ...
    # MIME types yang benar-benar executable
    BLOCKED_MIMES = {
    ...
    def evaluate(self, payload: DetectionPayload) -> RuleResult:
        ...
        if mime in self.BLOCKED_MIMES:
            return self._result(True, 80, f"MIME type executable diblokir: {mime}")
        if ext in self.BLOCKED_EXTS:
            return self._result(True, 80, f"Ekstensi executable diblokir: {ext}")
...
class SuspiciousFilenameRule(Rule):
    ...
    DOUBLE_EXT = re.compile(r"\.(pdf|doc|docx|xls|jpg|jpeg|png|zip)\.(exe|bat|cmd|ps1|dll)$", re.I)
    ...
    def evaluate(self, payload: DetectionPayload) -> RuleResult:
        ...
        if self.DOUBLE_EXT.search(fn):
            return self._result(True, 50, f"Double extension mencurigakan: {fn}")
...
class FileSizeAnomalyRule(Rule):
    """
    Limit diperbarui: 50 MB untuk semua user.
    Anon: 10 MB, Registered: 50 MB.
    """
    ...
    def evaluate(self, payload: DetectionPayload) -> RuleResult:
        ...
        # Limit yang diperbarui
        limit_mb    = 50 if payload.is_authenticated else 10
        limit_bytes = limit_mb * 1024 * 1024

        if size > limit_bytes:
            return self._result(True, 20,
                f"Ukuran file ({size/1024/1024:.1f} MB) melebihi batas {limit_mb} MB.")
        if size > limit_bytes * 0.9:
            return self._result(True, 10,
                f"Ukuran file ({size/1024/1024:.1f} MB) mendekati batas {limit_mb} MB.")
        return self._result(False, 0)
        ...
        try:
            ...
            if ent < self.MIN_ENTROPY:
                ...
                return self._result(True, score,
                    f"Entropy ciphertext rendah: {ent:.2f} bit/byte")
        except Exception:
            pass
        return self._result(False, 0)
...
class SuspiciousUserAgentRule(Rule):
    ...
    BOT_RE = re.compile(
        ...
        r"mechanize|libwww|okhttp|java/|masscan|sqlmap|nikto)", re.I
    )
...
class ComboAnonAbuseRule(Rule):
    ...
    def evaluate(self, payload: DetectionPayload) -> RuleResult:
        ...
        if not payload.expires_in_hours or payload.expires_in_hours > 24:
            weighted += 15; details.append("tanpa expiry wajar")
        if payload.num_recipients > 5:
            weighted += 10; details.append(f"{payload.num_recipients} penerima")
        if not payload.has_password:
            weighted += 5; details.append("tanpa password")
        if not payload.has_email_whitelist:
            weighted += 5; details.append("tanpa whitelist")
        if weighted >= 20:
            return self._result(True, weighted,
                f"Pola anon mencurigakan: {', '.join(details)}. Weighted: {weighted}")
        return self._result(False, 0)
...
class ComboMalwareDistributionRule(Rule):
    ...
    BOT_RE = re.compile(r"(python-requests|curl|wget|httpie|go-http|scrapy)", re.I)
    ...
    def evaluate(self, payload: DetectionPayload) -> RuleResult:
        ...
        if payload.file_size_bytes and payload.file_size_bytes > 1_000_000:
            weighted += 20; details.append(f"file {payload.file_size_bytes/1_000_000:.1f} MB")
        if not payload.is_authenticated:
            weighted += 15; details.append("anon user")
        ua = payload.user_agent or ""
        if not ua or self.BOT_RE.search(ua):
            weighted += 20; details.append("bot UA")
        if not payload.has_password:
            weighted += 10; details.append("tanpa password")
        if weighted >= 35:
            return self._result(True, weighted,
...
class MetadataAIDetector:
    ...
    def run(self, payload: DetectionPayload) -> DetectionResult:
        ...
        logger.info(
            ...
            f"Action: {result.action}, Rules: {result.rules_triggered}"
        )
        return result

    def run_and_save(self, payload: DetectionPayload, secret=None, ip_address: str = "") -> DetectionResult:
